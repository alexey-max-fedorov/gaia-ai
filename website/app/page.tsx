...
import { ENGINES, INTEGRATIONS, RELIABILITY, PERMISSION_MODES, URLS } from "@/lib/site";
              ...
              {PERMISSION_MODES.map((m) => (
                <div key={m.id} className="p-6 border border-[#1DD3B0]/12 bg-[#0D1526]/40">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-[var(--font-rajdhani)] text-lg font-bold tracking-[0.15em] text-[#E8EAF6]">{m.name}</span>
                    <span className="font-[var(--font-ibm-mono)] text-[7px] tracking-[0.3em] uppercase px-1.5 py-0.5 border text-[#1DD3B0]/70 border-[#1DD3B0]/25">{m.badge}</span>
                  </div>
                  <p className="font-[var(--font-inter)] text-xs text-[#6B7A94] leading-relaxed">{m.body}</p>
                </div>
              ))}
            </div>
          </div>
